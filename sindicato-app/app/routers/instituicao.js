...
module.exports = async function (app) {
    ...
    */
    ...
    // Rota para exibição da Lista de Municípios
    app.get('/app/' + rota + '/create', function (req, res) {
        if (!req.session.token || req.session.json.nivel != 'ADMIN') {
            res.redirect('/app/login');
        } else {
    ...
    app.get('/app/' + rota + '/municipio/:id', function (req, res) {
            municipios = [];
            request({
                url: process.env.API_HOST + "municipio/estado/" + req.params.id,
                method: "GET",
                json: true,
                headers: {
                    "content-type": "application/json",
